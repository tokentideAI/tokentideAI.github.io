...
![SIRL RLVR diagram](../cytTechBlog/figs/optharness/diagram.jpg){#fig-example}
...
# Retrieve the constraint matrix A as a scipy.sparse.csr_matrix
# Useful for evaluating sparse structure density and condition metrics
A = m.getA()
2. Infeasibility Root-Cause IsolationWhen an LLM produces contradictory constraints, runtime error messages are often insufficient. We can instruct the solver to compute an Irreducible Inconsistent Subsystem (IIS) to isolate the conflict:Pythonif m.Status == GRB.INFEASIBLE:
    # Compute Irreducible Inconsistent Subsystem (IIS)
    m.computeIIS() 
    # Export minimal conflicting constraint subset for reflection
    m.write("conflict_model.ilp")
3. Sensitivity Analysis & Economic SignalsFor linear programming tasks, dual metrics provide direct operational signals beyond primal decision variable values:Python# Retrieve shadow prices (dual variables) across constraints
pi_vals = m.getAttr('Pi', m.getConstrs())

# Retrieve reduced costs across decision variables
rc_vals = m.getAttr('RC', m.getVars())
By pairing solver backends with an IR-injection Harness engine, we establish a robust interface between probabilistic language models and deterministic mathematical solvers—enabling scalable RL training and dependable real-world deployments.
...
Inspired by multi-agent optimization frameworks such as Optimus*[@chen2026solver],
...
+--------+---------+
         ...
         v
+------------------+
| Harness Engine   |  (Executes in sandbox, extracts logs & metadata)
+--------+---------+
         |
         v
+------------------+
